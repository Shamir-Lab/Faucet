...
int main(int argc, char *argv[])
{
    ...
    int ASSEMBLE = 1;//if = 0, assemble.  If = 1, don't assemble. 
    ...
    if (sizeKmer > 128)
    {
        ...
        printf("Log of 2: %f \n", log(2));
      NBITS_PER_KMER = -log(fpRate)/log(2)/log(2); // needed to process argv[5]
   ...
   if(argc >=  5)
    {
      ...
      // int estimated_bloom_size = max( (int)ceilf(log2f(genome_size * NBITS_PER_KMER )), 1);
        uint64_t estimated_bloom_size = (uint64_t) (genome_size * NBITS_PER_KMER*(100-sizeKmer));
        printf("Estimated bloom size: %d .\n", (int)(genome_size*NBITS_PER_KMER*(100-sizeKmer)));
    ...
    printf("Max Memory: %d \n", max_memory);
    bloo1 = new Bloom((uint64_t)(max_memory*8000000));

    printf("Bits per kmer: %f \n", NBITS_PER_KMER);
...
/*
    ...
    */
    fprintf(stderr,"Program reached end. \n");
    return 0;
}
